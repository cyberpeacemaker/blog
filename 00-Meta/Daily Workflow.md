...
3. **Cursor daily inbox triage automation** runs on `main`, commits by phase, validates, then runs `bash scripts/finish-ai-task.sh` to push directly to `origin main` (see [[automation-prompt-suggestion]] and [[automation-pr-merge-policy]])
